...
class AcceptanceRuleController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'inspection_type' => ['required', Rule::in(array_keys(AcceptanceRule::INSPECTION_TYPES))],
            'sampling_method' => ['required', Rule::in(array_keys(AcceptanceRule::SAMPLING_METHODS))],
    ...
    public function update(Request $request, AcceptanceRule $acceptanceRule): JsonResource
    {
        $validated = $request->validate([
            ...
            'category_id' => 'nullable|exists:categories,id',
            'supplier_id' => 'nullable|exists:suppliers,id',
            'inspection_type' => ['sometimes', 'required', Rule::in(array_keys(AcceptanceRule::INSPECTION_TYPES))],
            'sampling_method' => ['sometimes', 'required', Rule::in(array_keys(AcceptanceRule::SAMPLING_METHODS))],
            'sample_size_percentage' => 'nullable|numeric|min:0|max:100',
            'aql_level' => 'nullable|string|max:20',
            'aql_value' => 'nullable|numeric|min:0|max:10',
            'criteria' => 'nullable|array',
